...
export default function LineChart({ onPriceChange, baseMint, quoteMint, timeType, untilDate }: Props) {
  const { colorMode } = useColorMode()

  const { t } = useTranslation()
  const chartCtrRef = useRef<HTMLDivElement>(null)
  const timeTypeRef = useRef<TimeType>(timeType)
  const chartRef = useRef<{ chart?: IChartApi; candle?: ISeriesApi<'Line'>; volume?: ISeriesApi<'Histogram'> }>({})
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartCtrRef.current!, {
      layout: { textColor: chartTextColor, background: { type: ColorType.Solid, color: 'transparent' }, fontFamily: 'Metrophobic' },
