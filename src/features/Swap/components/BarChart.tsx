...
export default function BarChart({ onPriceChange, baseMint, quoteMint, timeType, untilDate }: Props) {
  const { colorMode } = useColorMode()

  const { t } = useTranslation()
  const chartCtrRef = useRef<HTMLDivElement>(null)
  const timeTypeRef = useRef<TimeType>(timeType)
  const chartRef = useRef<{ chart?: IChartApi; candle?: ISeriesApi<'Bar'>; volume?: ISeriesApi<'Histogram'> }>({})
