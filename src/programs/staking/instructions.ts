...
import {
  getClaimRewardsAccounts,
  getCreateStakeEntryAccounts,
  getCreateStakePoolAccounts,
  getInitializeAccounts,
  getSetStakePoolRewardsAccounts,
  getStakeAccounts,
  getUnstakeAccounts
} from './accounts'
...
export async function initialize(admin: web3.Signer) {
  ...
  const tx = await program.methods.initialize().accounts(accounts).signers([admin]).rpc({ skipPreflight: true })
...
export async function setStakePoolRewards(
  stakePool: web3.PublicKey,
  admin: web3.Signer,
  rewardsRate: anchor.BN,
  depositFee: anchor.BN,
  withdrawFee: anchor.BN
) {
  let accounts = await getSetStakePoolRewardsAccounts(admin.publicKey, stakePool)
  const tx = await program.methods.setStakePoolRewards(rewardsRate, depositFee, withdrawFee).accounts(accounts).signers([admin]).rpc()
...
export async function createStakeEntry(staker: web3.Signer, stakePool: web3.PublicKey) {
  ...
  const tx = await program.methods.createStakeEntry().accounts(accounts).signers([staker]).rpc()

  await program.provider.connection.confirmTransaction(tx)
  return accounts.stakeEntry
}

export async function createStakeEntryIx(program: Program<Staking>, staker: web3.PublicKey, stakePool: web3.PublicKey) {
  let accounts = await getCreateStakeEntryAccounts(staker, stakePool)
  const ix = await program.methods.createStakeEntry().accounts(accounts).signers([]).instruction()
...
export async function stake(
  ...
  const tx = await program.methods.stake(amount).accounts(accounts).signers([staker]).rpc()
...
export async function createStakeIx(
  ...
  const ix = await program.methods.stake(amount).accounts(accounts).signers([]).instruction()
...
export async function createUnstakeIx(
  ...
  const ix = await program.methods.unstake(amount).accounts(accounts).signers([]).instruction()
...
export async function createClaimRewardsIx(
  ...
  let accounts = await getClaimRewardsAccounts(staker, stakePool, mintA)

  const ix = await program.methods.claimRewards().accounts(accounts).signers([]).instruction()
