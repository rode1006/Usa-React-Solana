...
function TokenInput(props: TokenInputProps) {
  ...
  return (
    <Box
      ...
      rounded={12}
      sx={ctrSx}
      borderRadius={topLeftLabel == 'From' ? '24px 24px 0px 0px' : '0px 0px 24px 24px'}
    >
      {disableTotalInputByMask ? (
        <Box
          rounded="inherit"
      ...
      >
        {/* top left label */}
        <Box fontSize="lg" fontWeight={500} style={{ background: colors.backgroundDark }}>
          {topLeftLabel}
        </Box>
        <Spacer />
      ...
      >
        <GridItem area="token" color={colors.textSecondary} fontWeight={500} fontSize={sizes.tokenSymbol}>
          <HStack
            bg={disableSelectToken ? undefined : colors.backgroundCoinBox}
            rounded={disableSelectToken ? undefined : 24}
            px={disableSelectToken ? undefined : 1}
            py={disableSelectToken ? undefined : 1}
          ...
          >
            ...
            {/* <Text fontSize={'16px'} color={isLight ? colors.secondary : colors.textPrimary}> */}
            <Text fontSize={'16px'} color={isLight ? colors.textPrimary : colors.textPrimary}>
              {token?.symbol || ' '}
            </Text>
            {disableSelectToken ? undefined : <ChevronDownIcon width={16} height={16} />}
          </HStack>
        </GridItem>
      ...
      <div>
        ...
        {!hideBalance && maxString && (
          <HStack spacing={0.5} color={colors.textTertiary} fontSize="sm" px="18px" pb="12px" justifyContent={'space-between'}>
            {/* {renderTopRightPrefixLabel()} */}
            <Box display={'flex'}>
              <Text color={colors.textPrimary}>Balance: </Text>
              <Text
                onClick={handleClickMax}
                cursor="pointer"
