...
import { PublicKey, VersionedTransaction, SystemProgram, TransactionMessage } from '@solana/web3.js'
...
import { SOLANA_CONNECTION, TIP_ACCOUNT, TIP_AMOUNT } from '@/utils/constant'
...
export const useLiquidityStore = createStore<LiquidityStore>(
  (set, get) => ({
    ...
    createPoolAct: async ({ pool, baseAmount, quoteAmount, startTime, onSent, onError, onFinally, onConfirmed }) => {
      const { raydium, programIdConfig, txVersion, publicKey, connection, signAllTransactions } = useAppStore.getState()
      if (!raydium) return ''
      ...
      try {
        ...
        const blockhash = await getRecentBlockHash(SOLANA_CONNECTION);
        ...
        const tipTx = new VersionedTransaction(txMessage);
        const signedTxs = await signAllTransactions([tipTx])
        ...
        if (confirmationResult.value.err) {
          console.log('debug confirm error::', confirmationResult.value.err);
          return ''
        } else {
          const computeBudgetConfig = await getComputeBudgetConfig()
